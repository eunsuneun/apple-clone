(() => {
  ...
  const setLayout = () => {
    ...
    // scroll-section-0의 반응형 캔버스 창 사이즈
    const heightRatio = window.innerHeight / 1080;
    sceneInfo[0].objs.canvas.style.transform = `translate3d(-50%, -50%, 0) scale(${heightRatio})`;
    sceneInfo[2].objs.canvas.style.transform = `translate3d(-50%, -50%, 0) scale(${heightRatio})`;
    // scroll-section-0의 캔버스 초기에 그려주기
    sceneInfo[0].objs.context.drawImage(sceneInfo[0].objs.videoImages[0], 0, 0);
  };
  const calcValues = (values, currentYOffset) => {
    let calculated;
    const scrollHeight = sceneInfo[currentScene].scrollHeight;
    const scrollRatio = currentYOffset / scrollHeight; // 현재 씬에서 스크롤된 범위 비율
    if (values.length === 3) {
      ...
      calculated = scrollRatio * (values[1] - values[0]) + values[0];
    }
    return calculated;
  };
  const playAnimation = () => {
    ...
    switch (currentScene) {
      ...
      case 2: {
        ...
        if (scrollRatio > 0.9) {
          ...
          // 캔버스 사이즈에 맞춰 innerWidth, innerHeight 설정
